...
//#define CC(x) OutputDebugString(x);
		...
		if ((super_block = read_super(dev)) && check_super(super_block))
		{
			...
			if (read_group_descs(dev, super_block, group_descs, num_group_descs))
			{
				if(alloc_new_fsd(result))
				{
					...
					rcache_init(&file_systems[index].rc, dev);
					bcache_init(&file_systems[index].cache, dev);
				}
			}
		}
	}
...
BOOL start_enum_dirs(FS_HANDLE fsh, char *dir_name, directory *&dirs/*out*/, DWORD &num_of_dir/*out*/)
{
	...
	if (inode_number = name_resolve(fs, dir_name))
	{
		...
		if (iget(fs, inode_number, &dir_inode) && S_ISDIR(dir_inode.i_mode))
		{
			...
			if(read_file(&dir_inode, 0, dir_inode.i_size, raw_dir, FALSE))
			{
				// прочитан файл директории
				DWORD current_num_of_dir = 0;
				ext2_dir_entry dir_entry;
				DWORD dir_pointer = 0;
				DWORD spec_dir = 0;
				result = TRUE; // if there are no subdirs...
				do
				{
					...
					if (dir_entry.inode)
					{
						if (spec_dir >= 2) // miss . &  ..
						{
							inode subdir_inode;
							result = FALSE;
							if (iget(fs, dir_entry.inode, &subdir_inode))
							{
								result = TRUE;
								if (S_ISDIR(subdir_inode.i_mode))
								{
				...
				} while (current_num_of_dir < num_of_dir && dir_pointer < dir_inode.i_size);
			}
			delete [] raw_dir;
		}
	}
		...
		if (iget(fs, inode_number, &dir_inode) && S_ISDIR(dir_inode.i_mode))
		{
			...
			if (read_file(&dir_inode, 0, dir_inode.i_size, raw_dir, FALSE))
			{
				...
				while (!end_of_dir)
				{
					...
					if (!end_of_dir) MoveMemory(&dir_entry, raw_dir + dir_pointer, sizeof(dir_entry));
				}
...
BOOL read_file(FS_HANDLE fsh, char *src, BYTE *buffer, DWORD start, DWORD length)
{
	...
	if (inode_num = name_resolve(fs, src))
	{
		...
		{
			if (read_file(&inode_file, start, length, buffer, FALSE))
                result = TRUE;
		}
	}
	return result;
...
BOOL free_fsd(FS_HANDLE fs_handle)
{
	...
	if (file_systems[index].dev != NULL)
	{
		...
		bcache_done(&file_systems[index].cache);
...
BOOL resolve_sym_link(FS_HANDLE fsh, char *sym_link, char *buf/*out*/, DWORD buflen)
{
	...
	if (inode_num = name_resolve(fs, sym_link))
	{
		...
		if (iget(fs, inode_num, &inode_sym_link) && inode_sym_link.i_size < buflen/* < because zero */)
		{
			...
			{
				if(read_file(&inode_sym_link, 0, fs->block_size, block, FALSE))
				{
					link = (char *)block;
					result = TRUE;
				}
...
{
	file_system *fs1 = &file_systems[fsh2index(fsh1)];
	ext2_super_block *sb1 = fs1->super_block;
	file_system *fs2 = &file_systems[fsh2index(fsh2)];
	ext2_super_block *sb2 = fs2->super_block;
	return sb1->s_mtime == sb2->s_mtime &&
		sb1->s_wtime == sb2->s_wtime &&
		sb1->s_mnt_count == sb2->s_mnt_count;
}
